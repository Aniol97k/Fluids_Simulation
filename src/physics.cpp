...
static int lastTotalWaves;
static bool lastMultipleWaves;

//Arrays for the waves
static float *frequencyArray;
static float *amplitudeArray;
glm::vec3 *waveVectorArray;
...
void GUI() {
	...
	ImGui::SliderFloat("Amplitude", &amplitude2, 0.05f, 0.09f);	ImGui::Spacing(); ImGui::Spacing(); ImGui::Spacing(); ImGui::Spacing();
...
void checkChanges() { if (lastHeight != height || lastTime != resetTime || lastFrequency != frequency || lastAmplitude != amplitude  || lastMass != mass || lastRadius != radius  || lastDensity != density) PhysicsInit(); } //Check variables changes to reset the simulation
...
void PhysicsUpdate(float dt) {
	...
	switch (multipleWaves) {
		...
		case true:
			...
			for (int i = 0; i < totalVertex; i++) { //Applying more than one Gerstner waves
